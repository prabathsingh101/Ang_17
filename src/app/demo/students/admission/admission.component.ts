...
import { Admission, Registration } from '../registration/models/registration';
...
export default class AdmissionComponent implements OnInit, OnDestroy {
  constructor(
    ...
    private ref: MatDialogRef<AdmissionComponent>
  ) {}
  ...
  onSubmit() {
    ...
    if (this.forms.valid) {
      ...
      this.admission = {
        ...
        registrationdate: this.editdata.registrationdate
      };
      ...
      forkJoin({
        postAdmission: this.admSvc.Post(this.admission),
        putRegistration: this.registrationSvc.PUT(this.inputdata.id, this.registration)
      }).subscribe((results: any) => {
        if (results.postAdmission.StatusCode === 201) {
          this.closepopup();
        }
      },(error) => {
